...
"""
【交通費申請 自動点検システム v4 (出張届対応版)】

■ 概要
従来の経費申請（Appform）に加え、「出張届及精算承認書」（Genesheet）の運賃点検に対応しました。
itemTable内の「運賃」項目を特定し、詳細をPOSTで取得してからAI判定を行います。
"""
...
def main():
    ...
    if app_table:
        ...
        for app in target_apps:
            ...
            for row in detail_table["rows"]:
                ...
                row_detail = fetch_json(row["日付"]["url"], token)
                ...
                if row_fields.get("AI判定") and row_fields["AI判定"].get("value") and row_fields["AI判定"]["value"] not in ["NG/Unknown", "Pending"]:
                    continue
                if db_id in blacklist: continue
                ...
                print(f"AI調査中 (App): {date_val} {route_key}...")
                ...
                if ai_res:
                    ...
                    post_res = post_data_to_sv(f"{BASE_URL}/ad/Appdetail/sv", token, post_payload)
                    
                    new_results.append({"id": db_id, "text": f"| {app_no:<5} | {date_val:<10} | {app['申請者']['text']:<8} | {route_key:<20} | {unit_price:<6} | {ai_correct:<6} | {status:<10} | {reason} |"})
                    processed_ids.append(db_id)
                
            app_cache[app_no] = last_updated
            app_cache_updated = True

    # --- 2. 出張届及精算承認書 (GenesheetTable) の処理 ---
    print("出張届及精算承認書を処理中...")
    ...
    if trip_table:
        ...
        for trip in target_trips:
            ...
            for row in item_table["rows"]:
                ...
                item_detail_res = post_data_to_sv(f"{BASE_URL}/ad/itemTable/detail", token, {"itemTableParams": json.dumps(params_for_detail),"head": "itemTable","kind": "出張届及精算承認書" })
                item_detail = json.loads(item_detail_res)
                item_fields = item_detail["forms"][0]["fields"]
                
                db_id = item_fields.get("id", {}).get("value") or f"trip_{app_no}_{data_ix}"
                if item_fields.get("AI判定") and item_fields["AI判定"].get("value") and item_fields["AI判定"]["value"] not in ["NG/Unknown", "Pending"]:
                    continue
                if db_id in blacklist: continue

                # 運賃情報の抽出 (itemTableのフィールド名に合わせる)
                date_val = item_fields.get("利用日", {}).get("value", "")
                from_st = item_fields.get("出発", {}).get("value", "")
                to_st = item_fields.get("到着", {}).get("value", "")
                amount_val = item_fields.get("金額", {}).get("value", "0")
                payee = item_fields.get("支払先", {}).get("value", "")
                content = item_fields.get("内容", {}).get("value", "")
                ...
                print(f"AI調査中 (Trip): {date_val} {route_key}...")
                ai_res = verify_with_ai(route_key, date_val, unit_price, is_round, payee)
                if ai_res:
                    thought, status, reason, last_rev = ai_res["thought"], ai_res["status"], ai_res["reason"], ai_res["last_revision_date"]
                    ai_correct = ai_res["correct_fare"]
                    ...
                    post_payload = {f_info["name"]: f_info["value"] for f_name, f_info in item_fields.items() if isinstance(f_info, dict) and "name" in f_info}
                    post_payload.update({"aijadge": status, "aifare": ai_correct, "aicomment": reason + "\n[算出根拠]:\n" + thought, "post": "true"})
                    # 保存先URLは経費精算と同じパターンと仮定
                    post_data_to_sv(f"{BASE_URL}/ad/itemTable/sv", token, post_payload)
                    
                    new_results.append({"id": db_id, "text": f"| {app_no:<5} | {date_val:<10} | {trip['申請者']['text']:<8} | {route_key:<20} | {unit_price:<6} | {ai_correct:<6} | {status:<10} | {reason} |"})
                    processed_ids.append(db_id)
    ...
    if new_results or app_cache_updated:
        ...
        if app_cache_updated:
            with open(APP_CACHE_FILE, "w", encoding="utf-8") as f: json.dump(app_cache, f, indent=2, ensure_ascii=False)
        if new_results:
